...
int save_macsn(void)
{
	...
	if ((m.sn[0] != 0xff) && (env_get("serial#") == NULL)) {
		env_set("serial#", (char *)m.sn);
	}
...
static void update_crc(void)
{
	m.version = MACSN_FORMAT_VERSION;
...
int do_macsn(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	case 's':	/* serial number */
		if (!((m.sn[0] == 0xff) || (m.sn[0] == 0x00))) {
			printf("Serial Number already set\n");
		} else {
			memset(m.sn, 0, sizeof(m.sn));
			strncpy((char *)m.sn, argv[2], sizeof(m.sn) - 1);
			update_crc();
		}
		break;
